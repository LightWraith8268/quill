...
export const WORKFLOWS: WorkflowDef[] = [
  ...
  {
    id: "reverse_canon",
    title: "Reverse-Canon Backfill",
    description:
      "When a later book contradicts an earlier book's detail, propose minimal back-edits to align canon. Diagnose-only; user applies changes manually.",
    agent: "claude",
    fields: [
      { name: "earlier_excerpt", label: "Earlier book passage (the contradicting detail)", kind: "textarea", rows: 6, required: true },
      { name: "later_excerpt", label: "Later book passage (the new canon)", kind: "textarea", rows: 6, required: true },
      { name: "anchor", label: "What's the contradicted fact?", kind: "text", required: true, placeholder: "e.g. Riko's age in Book 1" },
    ],
    buildPrompt: (i) => [
      `REVERSE-CANON BACKFILL.`,
      ``,
      `Two passages disagree. The LATER passage is canonical. Propose minimal edits to the EARLIER passage so the disagreement disappears, while preserving every other beat of the earlier scene.`,
      ``,
      `Output format:`,
      `1. **Diagnosis** — one paragraph naming the conflict.`,
      `2. **Proposed earlier-text edits** — show before/after on each affected line, smallest possible delta.`,
      `3. **Ripple check** — what else in the earlier book might need alignment? Bullet list.`,
      ``,
      `Anchor: ${i.anchor}`,
      ``,
      `=== EARLIER PASSAGE (will be edited) ===`,
      i.earlier_excerpt ?? "",
      ``,
      `=== LATER PASSAGE (canonical) ===`,
      i.later_excerpt ?? "",
    ].join("\n"),
  },
  {
    id: "comp_finder",
    title: "Comp Title Suggestions",
    description:
      "Suggest published comp titles based on opening chapter + tone. Real titles only. Best with Codex (broad lit knowledge).",
    agent: "codex",
    fields: [
      { name: "opening", label: "Opening chapter", kind: "textarea", rows: 12, required: true },
      { name: "tradition", label: "Genre tradition (optional)", kind: "text", placeholder: "literary sf, urban fantasy with engineering magic, etc" },
      { name: "exclude", label: "Books to exclude (already considered)", kind: "text" },
    ],
    buildPrompt: (i) => [
      `COMP TITLE SUGGESTIONS.`,
      ``,
      `Read the opening. Suggest 6-8 published comp titles (real books only, last ~10 years preferred for query letters). For each, give:`,
      `- **Title** by Author`,
      `- **Why** — 1 sentence linking specific craft / tonal / structural similarity to the opening.`,
      ``,
      `Don't pick the obviously-best-selling neighbors; pick titles that share *craft* with this manuscript. Avoid books in the exclude list.${i.tradition?.trim() ? `\n\nTradition: ${i.tradition.trim()}` : ""}${i.exclude?.trim() ? `\n\nExclude: ${i.exclude.trim()}` : ""}`,
      ``,
      `=== OPENING ===`,
      i.opening ?? "",
    ].join("\n"),
